...
- [ ] T003 [P] Add a transitive-dep license audit document at `docs/licenses.md` listing every module in `go.sum` with its SPDX license identifier and the source-of-truth URL. Generated by a tiny script that the doc itself documents (so future contributors can re-run it after adding deps). Manual cross-check against constitution §IX (Apache/MIT/BSD/MPL 2.0 only — flag anything else). Commit: `docs(licenses): add transitive license audit for core-loop deps`.
