...
- [ ] T067 [US6] Vendor frontend assets under `web/static/`: `htmx.min.js` (HTMX 2.0.x — pin a specific minor version, ~14KB), `alpine.min.js` (Alpine 3.13.x — pin, ~16KB), and `app.css` (output of `tailwindcss -i web/_src/app.css -o web/static/app.css --minify` run locally; `web/_src/app.css` holds the Tailwind directives + custom theme tokens for the sage/teal palette and Geist/Instrument-Serif fonts). Add `web/static/README.md` documenting exact versions and regeneration steps. Commit: `chore(web): vendor htmx, alpine, and tailwind output for slim dashboard`.
...
- [ ] T070 [US6] Add `internal/server/ui.go` with handlers: `GET /` (302 to `/ui/`), `GET /ui/` (renders `index.html` with `Status` data from the same aggregator used by `/api/v1/system/status`), `GET /ui/services` (renders `services_table.html` fragment), `GET /static/*` (serves files from `web.StaticFS` via `http.FileServer`). Middleware `RequireAuthOrUnix` bypasses the Bearer-token check when `server.IsUnixListener()` returns true; otherwise enforces the same `Authorization` header as `/api/v1`. Wire these routes in `internal/server/routes.go` (extend T043). Commit: `feat(server): add slim dashboard routes with Unix-socket auth bypass`.

- [ ] T071 [US6] Add `tests/e2e/dashboard_test.go` (build-tagged `e2e`) covering SC-008: start daemon, `proxa up` an nginx service with 2 replicas, `curl --unix-socket` to `/ui/`, parse the returned HTML with `golang.org/x/net/html` (already a transitive of stdlib `net/http` ecosystem; if not, accept the dep as auditable BSD-3), assert the table row for the service exists with desired=2/actual=2. Then `proxa down`, sleep 6s, GET `/ui/services` (the fragment), assert the row is gone or shows actual=0. Commit: `test(e2e): cover slim dashboard renders services (SC-008)`.
