...
- [X] T072 [US6] Update `web/README.md` (replace the placeholder content from feature 000) to document the actual contents and regeneration of vendored assets. Add a section to `CLAUDE.md` under "Tech stack" with: how to bump HTMX/Alpine pins, how to regenerate `app.css` via `tailwindcss` standalone CLI (one-shot, never in CI), and a note that the slim v0.0 dashboard intentionally has no login form. Commit: `docs(web): document slim dashboard assets and regeneration`.
