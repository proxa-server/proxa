...
- **2026-05-27** — feature 009 (probe-events-host-logs / v0.4.5) audit refresh: **no-op confirmation**. New code reuses `events.Store` via a narrow `probe.EventSink` interface (no events package import in probe) + the existing `Runtime.StreamLogs` for host-container logs + one new HTML template. Zero new modules.
- **2026-05-27** — feature 008 (container-ui / v0.4.4) audit refresh: **no-op confirmation**. Pure additions: one new `Runtime` interface method (`ListAllContainers`) implemented against the already-vendored `docker/docker` client, plus host-container HTTP handlers using `net/http` + `encoding/json` from stdlib, plus one new HTML template. No new modules. `go list -m all` unchanged.
- **2026-05-27** — feature 007 (architectural-foundations / v0.4.3) audit refresh: **no-op confirmation**. New code is pure-stdlib (`internal/events`, `internal/state`, `internal/cluster`, `internal/plugin` all use only `database/sql`, `context`, `sync`, `log/slog`, `errors`, `time`, `fmt`). The TOML parser meta-block additions use the already-imported `BurntSushi/toml`. SQLite migration v2 (events table) uses the same `modernc.org/sqlite` driver from v0.1. Total `go list -m all` count unchanged at ~932. `go mod tidy` round-trip stable. No §IX action needed.
- **2026-05-26** — feature 006 (test-foundation-public-images / v0.4.2) audit refresh. ONE new `tool`-directive entry: `github.com/goreleaser/goreleaser/v2` (MIT) for the release pipeline (multi-arch images + binary archives + checksums). Brought in a LARGE transitive surface (~800 modules) because Goreleaser pulls in AWS / GCP / Cloudflare SDKs for its uploaders, plus sigstore/cosign helpers — none of these are linked into the proxa or proxa-agent binaries; they exist only in the build-time tool graph. Total `go list -m all` count now ~932 (vs 124 at v0.4.1). Production binary closure unchanged — `go list -deps ./cmd/proxa` returns the same 10 direct + transitive set as v0.4.1, verified by binary-size delta of −75KB (-0.27% from v0.4.1 baseline 27450050 bytes). All new transitives Apache-2.0 / MIT / BSD per pkg.go.dev spot-checks; spot-checked Goreleaser (MIT), sigstore/cosign (Apache-2.0), and the AWS SDK family (Apache-2.0). No §IX violations.
