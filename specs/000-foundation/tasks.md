...
- [ ] T015 [P] [US1] Create `internal/version/version.go` exporting `var Version = "dev"`, `var Commit = "unknown"`, `var BuildDate = "unknown"` (mutable so `-ldflags "-X internal/version.Version=..."` can override at build time). Doc comment on each var. Imports: none. Commit: `feat(version): add build-time version vars`.
