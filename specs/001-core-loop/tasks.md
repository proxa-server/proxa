...
- [ ] T021 Replace the `noopPolicyEngine` in `internal/auth/policy.go` with a real `dbPolicyEngine` that consults `StateStore.ListPoliciesFor`. Authorize logic: `RoleAdmin` matches every (verb, kind, project); `RoleEditor` matches CRUD on services/jobs/secrets within scoped project; `RoleViewer` matches get/list within scoped project; `RoleAgent` matches get on services/jobs in any project + update on Heartbeat for own node. Empty `Project` for project-scoped kinds → `ErrForbidden`. Commit: `feat(auth): replace noop PolicyEngine with StateStore-backed impl`.

- [ ] T022 [P] Add `internal/auth/policy_test.go` table-driven across the role × verb × kind matrix; uses a fake StateStore returning fixed Policy lists. Commit: `test(auth): cover PolicyEngine role matrix`.
...
- [ ] T025 Create `internal/runtime/docker/security.go` with `applySecurityProfile(spec runtime.ContainerSpec) (host *container.HostConfig, cfg *container.Config)` that calls `security.Apply(spec.Security)` then maps to Docker's `HostConfig.CapAdd`/`CapDrop`/`SecurityOpt`/`ReadonlyRootfs` and `Config.User`. Constitution §II is enforced HERE — no caller can bypass. Commit: `feat(runtime/docker): apply SecurityProfile defaults to container HostConfig`.

- [ ] T026 [P] Add `internal/runtime/docker/security_test.go` table-driven: zero `SecurityProfile{}` yields `CapDrop=[ALL]` + `no-new-privileges:true` in output; explicit `AllowRoot+User=root` honored; the test does NOT touch Docker — it exercises the spec-mapping function directly. Commit: `test(runtime/docker): verify SecurityProfile mapping to Docker config`.
...
- [ ] T033 Create `internal/reconciler/hash.go` with `Hash(types.TaskDef) string` returning `"sha256:" + hex(sha256(canonicalJSON(spec)))`. CanonicalJSON sorts keys recursively. Imports stdlib only. Commit: `feat(reconciler): add canonical-JSON SHA-256 hash for TaskDefs`.

- [ ] T034 [P] Add `internal/reconciler/hash_test.go` covering: identical specs hash equal across map ordering and slice append-order edge cases; different image yields different hash; semantically equivalent omitempty fields hash equal. Commit: `test(reconciler): cover hash determinism across map orderings`.

- [ ] T035 Create `internal/reconciler/diff.go` with the pure function `Compute(desired []types.Service, actual []runtime.ContainerInfo) []Action`. Action types: `CreateContainer{Project,Service,Replica,Spec}`, `RemoveContainer{ID}`, `ReplaceContainer{ID,Project,Service,Replica,Spec}` (replace = remove + create per the naive strategy from Complexity Tracking; reason recorded as `spec_hash drift` or `image change`). Action ordering is deterministic (project, service, replica). Commit: `feat(reconciler): add pure diff function (desired vs actual → actions)`.
...
- [ ] T041 Create `internal/server/server.go` with `Server` struct + `New(cfg, store, runtime, recon, auth) *Server` + `Start(ctx)` (binds the configured listener, sets socket mode 0660 if Unix) + `Shutdown(ctx)` (graceful). Listener URL parsing: `unix:///path` → Unix socket; `tcp://host:port` → TCP. Commit: `feat(server): add HTTP server lifecycle with Unix/TCP listener support`.
...
- [ ] T051 [US1] Create `internal/cli/up.go` implementing `proxa up <file>`: read the file, call `parser/toml.Parse` + `Validate`, then `client.UpsertService(taskdef)`. Print the result. Exit 1 on validation error with the error code prefix from the grammar contract. Commit: `feat(cli): implement proxa up`.
...
**Checkpoint**: SC-005 passes. All five user stories independently testable.
...
- [ ] T063 [P] Add an integration test at `tests/e2e/no_daemon_test.go` (build-tagged `e2e`) covering SC-007: with the server NOT running, `proxa up` exits 1 with stderr matching `cannot reach proxa server` and a hint to `proxa server`. Commit: `test(e2e): cover clear error when daemon absent (SC-007)`.
...
- **Phases 3–7 (user stories)**: each can start once Phase 2 completes. Within each story: implementation task → unit test → integration test, in that order.
- **Phase 8 (Polish)**: T062/T063/T064 in parallel after Phase 7. T065 after all dep changes settle. T066 last.

### User-story dependencies

US1 (deploy) and US2 (auto-restart) are both P1 and form the MVP. US2 has no production code of its own — it's verified by an integration test on top of US1's deploy. US3, US4, US5 are P2/P3 conveniences; each is independent and can ship in any order after US1/US2.
...
2. Phase 2 (Foundational) — 47 tasks. The bulk of the feature.
3. Phase 3 (US1 deploy) — 3 tasks.
4. Phase 4 (US2 auto-restart) — 1 task.

That gets you a working "deploy + keep alive" orchestrator. ~54 tasks. From there:
...
8. Phase 8 (Polish) — 5 tasks. Closes SC-006/SC-007 and validates.
...
- After Phases 3–7: each user story's integration test passes when run with `make test-e2e`.
