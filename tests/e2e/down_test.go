...
func TestSC_004_ProxaDownRemovesContainers(t *testing.T) {
	...
	waitForCount(t, "downtest", 2, 15*time.Second)
	...
	if out, err := runProxa(t, dir, "down", "downtest"); err != nil {
		t.Fatalf("down: %v\n%s", err, out)
	}
	waitForCount(t, "downtest", 0, 15*time.Second)
}
...
func TestProxaDownIsIdempotent(t *testing.T) {
	...
	if out, err := runProxa(t, dir, "down", "does-not-exist"); err != nil {
